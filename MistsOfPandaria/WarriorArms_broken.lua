...
spec:RegisterResource( 1, {
    ...
    mortal_strike_regen = {
        last = function ()
            return (state.last_cast_time and state.last_cast_time.mortal_strike) or 0
        end,
    ...
    charge_rage = {
        last = function ()
            return (state.last_cast_time and state.last_cast_time.charge) or 0
        end,
        interval = 20, -- Charge cooldown
        value = function()
            return state.last_ability == "charge" and glyph.bull_rush.enabled and 25 or 10
        end,
    },
    ...
    victory_rush_rage = {
        last = function ()
            return (state.last_cast_time and (state.last_cast_time.victory_rush or state.last_cast_time.impending_victory)) or 0
        end,
        interval = 1,
        value = function()
...
spec:RegisterAbilities( {
    ...
    charge = {
        ...
        cast = 0,
        cooldown = function() 
            if talent.juggernaut.enabled then
                return 12
            end
            return 20 
        end,
        charges = function()
            if talent.double_time.enabled then
                return 2
            end
            return 1
        end,
